                ...
                for (int i = 0; i < imgPaletteEntries.Length; i++)
                {
                    if (!pixColormap.AddColor(imgPaletteEntries[i].ToPixColor()))
                    {
                        throw new InvalidOperationException(
                            String.Format("Failed to add colormap entry {0}.", i)
                        );
                    }
                }
        ...
        private int GetPixDepth(PixelFormat pixelFormat)
        {
            switch (pixelFormat)
            {
                ...
                default:
                    throw new InvalidOperationException(
                        String.Format(
                            "Source bitmap's pixel format {0} is not supported.",
                            pixelFormat
                        )
                    );
            }
        ...
        private unsafe void TransferDataFormat24bppRgb(BitmapData imgData, PixData pixData)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    byte red = pixelPtr[2];
                    PixData.SetDataFourByte(
                        pixLine,
                        x,
                        BitmapHelper.EncodeAsRGBA(red, green, blue, 255)
                    );
                }
            }
        }
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    PixData.SetDataFourByte(
                        pixLine,
                        x,
                        BitmapHelper.EncodeAsRGBA(red, green, blue, alpha)
                    );
                }
            }
        }
