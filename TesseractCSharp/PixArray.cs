...
namespace TesseractCSharp
{
    ...
    public class PixArray : DisposableBase, IEnumerable<Pix>
    {
        ...
        {
            var pixaHandle = NativeLeptonicaApi.pixaReadMultipageTiff(filename);
        ...
        {
            var pixaHandle = NativeLeptonicaApi.pixaCreate(n);
            if (pixaHandle == IntPtr.Zero)
            {
        ...
        private PixArray(IntPtr handle)
        {
            ...
            _count = NativeLeptonicaApi.pixaGetCount(_handle);
        }
        ...
        public bool Add(Pix pix, PixArrayAccessType copyflag = PixArrayAccessType.Clone)
        {
            ...
            int result = NativeLeptonicaApi.pixaAddPix(_handle, pix.Handle, copyflag);
            if (result == 0)
            {
                _count = NativeLeptonicaApi.pixaGetCount(_handle);
            }
        ...
        public void Remove(int index)
        {
            ...
            VerifyNotDisposed();
            if (NativeLeptonicaApi.pixaRemovePix(_handle, index) == 0)
            {
                _count = NativeLeptonicaApi.pixaGetCount(_handle);
            }
        }
        ...
        {
            VerifyNotDisposed();
            if (NativeLeptonicaApi.pixaClear(_handle) == 0)
            {
                _count = NativeLeptonicaApi.pixaGetCount(_handle);
            }
        }
        ...
        public Pix GetPix(int index, PixArrayAccessType accessType = PixArrayAccessType.Clone)
        {
            ...
            var pixHandle = NativeLeptonicaApi.pixaGetPix(_handle, index, accessType);
            if (pixHandle == IntPtr.Zero)
            {
                throw new InvalidOperationException(
                    String.Format("Failed to retrieve pix {0}.", pixHandle)
                );
            }
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            NativeLeptonicaApi.pixaDestroy(ref handle);
            _handle = new HandleRef(this, handle);
        }
